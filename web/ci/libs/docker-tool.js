...
function compileAndBuild(params) {
    ...
    if (paramsHelper.isWeb()){
        ...
        compileCommand= 'docker run -i --rm -u root --name nodejs-project -v /root/.npm:/root/.npm -v '+ workPath + ':/usr/src/mynode:Z -w /usr/src/mynode node:8.10.0-slim sudo npm install';
        exec(compileCommand);
        compileCommand= 'docker run -i --rm -u root --name nodejs-project -v /root/.npm:/root/.npm -v '+ workPath + ':/usr/src/mynode:Z -w /usr/src/mynode node:8.10.0-slim sudo npm run build';
        exec(compileCommand);
        return true;
    }else{
