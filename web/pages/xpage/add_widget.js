...
@inject('pagesStore') @inject('widgetsStore')
@observer
export default class DetailPage extends React.Component {
    ...
    onFinish = values => {
        ...
        this.finalKeys.forEach(function(item){
            let itemId = item.id;//= that.props.interfacesStore.dataObject.list[item];
            that.Store().addWidget(pageId,itemId,function(value){
                console.log('success to add widget id:' + value);
            });
        });
    ...
    componentDidMount() {
        ...
        this.Store().queryById(id,function(page){
            that.props.widgetsStore.queryAll();
        });
    ...
    render() {
        ...
        let widgets = this.props.widgetsStore.dataObject.list;
        return (
            <div>
                <Card size="small" title="基本信息" style={{ width: 800 }}  >
                <SelectTransfer data={widgets} filterData={that.filterData} onItemClick={that.onItemClick} handleChange={that.handleChange}></SelectTransfer>
