...
@inject('modulesStore') @inject('releasesStore')
@observer
export default class DetailPage extends React.Component {
    ...
    onFinish = values => {
        ...
        this.finalKeys.forEach(function(item){
            let moduleId = item.id;//= that.props.interfacesStore.dataObject.list[item];
            that.Store().addModule(releaseId,moduleId,function(value){
    ...
    componentDidMount() {
        ...
        this.Store().queryById(id,function(release){
            that.props.modulesStore.queryByProjectId(release.projectId);
        });
    ...
    render() {
        ...
        let modules = this.props.modulesStore.dataObject.list;
        return (
            <div>
                <Card size="small" title="基本信息" style={{ width: 800 }}  >
                <SelectTransfer data={modules} filterData={that.filterData} onItemClick={that.onItemClick} handleChange={that.handleChange}></SelectTransfer>
